...
const QuoteDetailModal = ({
  ...
  // ─── Estado local del quote para reflejar cambios inmediatamente ──────────
  ...
  const handleStatusChange = (status) => {
    if (status === 'CANCELLED') {
      // Para CANCELLED se necesita un motivo (el back lo requiere)
      openPrompt({
        title: 'Cancelar cotización',
        message: '¿Estás seguro que querés cancelar? Ingresá el motivo (requerido).',
        placeholder: 'Ej: Cliente desistió del proyecto...',
        variant: 'danger',
        ...
        onConfirm: async (reason) => {
          ...
          if (!reason?.trim()) {
            openConfirm({
              ...
              message: 'Debés ingresar un motivo para cancelar la cotización.',
              variant: 'warning',
          ...
          try {
            await onUpdateStatus(quote.id, status, reason.trim());
            // Actualizar estado local inmediatamente
            setQuote((prev) => ({ ...prev, status, rejection_reason: reason.trim() }));
          } finally {
      ...
      openConfirm({
        ...
        message: `Se va a cambiar el estado a "${STATUS_LABELS[status] || status}".`,
        variant: 'default',
        ...
        onConfirm: async () => {
          ...
          try {
            ...
            // Actualizar estado local + fecha correspondiente
            setQuote((prev) => ({
              ...prev,
              status,
  ...
  const handleReview = (paymentType, action) => {
    if (action === 'reject') {
      openPrompt({
        ...
        message: `Ingresá el motivo del rechazo del comprobante de ${paymentType === 'deposit' ? 'seña' : 'pago final'}.`,
      ...
      openConfirm({
        ...
        message: `¿Confirmás que el comprobante de ${paymentType === 'deposit' ? 'seña' : 'pago final'} es válido?`,
        variant: 'default',
        ...
        onConfirm: async () => {
          ...
          setActionLoading(true);
          try {
            await onReviewProof(quote.id, paymentType, action, '');
            // Actualizar estado local
            setQuote((prev) => ({
              ...prev,
              ...(paymentType === 'deposit'
  ...
  const handleDelete = () => {
    openConfirm({
      ...
      message: `Se eliminará permanentemente la cotización ${quote.quote_number}. Esta acción no se puede deshacer.`,
      variant: 'danger',
  ...
  return (
    <>
      {/* ── Dialog custom (reemplaza confirm/prompt del browser) ── */}
      <AdminDialog
        open={dialog.open}
        type={dialog.type}
      ...
      <div className="qdm-overlay" onClick={onClose}>
        <div className="qdm-container" onClick={e => e.stopPropagation()}>
          ...
          <div className="qdm-body">
            ...
            {Object.keys(details).length > 0 && (
              <div className="qdm-section">
                ...
                <div className="qdm-details-grid">{renderDetails()}</div>
              </div>
            )}
            ...
            {quote.quoted_amount && (
              <div className="qdm-section">
                <div className="qdm-section-title-row">
                  ...
                  {quote.budget_pdf && (
                    <a href={`${API_URL}${quote.budget_pdf}`} target="_blank" rel="noopener noreferrer" className="qdm-pdf-btn">
                      <FileEarmarkText size={14} />Ver PDF
                    </a>
                  )}
                </div>
                <div className="qdm-budget-table">
                  {quote.materials_subtotal && (
                    <div className="qdm-budget-row"><span>Materiales</span><span>{formatCurrency(quote.materials_subtotal)}</span></div>
                  )}
                  {quote.labor_subtotal && (
                    <div className="qdm-budget-row"><span>Mano de obra</span><span>{formatCurrency(quote.labor_subtotal)}</span></div>
                  )}
                  ...
                  {quote.tax_amount && (
                    <div className="qdm-budget-row"><span>IVA ({quote.tax_percentage}%)</span><span>{formatCurrency(quote.tax_amount)}</span></div>
                  )}
                  ...
                  {quote.deposit_amount && (
                    <div className="qdm-budget-row deposit">
                      <span>Seña ({quote.deposit_percentage}%)</span><span>{formatCurrency(quote.deposit_amount)}</span>
                    </div>
                  )}
                ...
                {quote.estimated_completion_days && (
                  <p className="qdm-completion">Tiempo estimado: <strong>{quote.estimated_completion_days} días hábiles</strong></p>
                )}
              </div>
            )}
              ...
              <div className="qdm-section">
                ...
                <div className="qdm-proof-row">
                  <div className="qdm-proof-left">
                    ...
                    <StatusBadge status={quote.deposit_payment_status || 'PENDING'} config={PROOF_STATUS_CONFIG} />
                  </div>
                  <div className="qdm-proof-actions">
                    {quote.deposit_proof_url && (
                      <a href={`${API_URL}${quote.deposit_proof_url}`} target="_blank" rel="noopener noreferrer" className="qdm-proof-view-btn">
                        Ver comprobante
                      </a>
                    )}
                    {quote.deposit_payment_status === 'PROOF_UPLOADED' && (
                      <>
                        <button className="qdm-proof-approve-btn" onClick={() => handleReview('deposit', 'approve')} disabled={actionLoading}>
                          <CheckCircle size={14} /> Aprobar
                        </button>
                        <button className="qdm-proof-reject-btn" onClick={() => handleReview('deposit', 'reject')} disabled={actionLoading}>
                ...
                {(quote.final_proof_url || quote.final_payment_status !== 'PENDING') && (
                  <div className="qdm-proof-row">
                    <div className="qdm-proof-left">
                      ...
                      <StatusBadge status={quote.final_payment_status || 'PENDING'} config={PROOF_STATUS_CONFIG} />
                    </div>
                    <div className="qdm-proof-actions">
                      {quote.final_proof_url && (
                        <a href={`${API_URL}${quote.final_proof_url}`} target="_blank" rel="noopener noreferrer" className="qdm-proof-view-btn">
                          Ver comprobante
                        </a>
                      )}
                      {quote.final_payment_status === 'PROOF_UPLOADED' && (
                        <>
                          <button className="qdm-proof-approve-btn" onClick={() => handleReview('final', 'approve')} disabled={actionLoading}>
                            <CheckCircle size={14} /> Aprobar
                          </button>
                          <button className="qdm-proof-reject-btn" onClick={() => handleReview('final', 'reject')} disabled={actionLoading}>
                            <XCircle size={14} /> Rechazar
                          </button>
                        </>
                      )}
            ...
            {quote.rejection_reason && (
              <div className="qdm-section">
                <div className="qdm-section-title">Motivo</div>
                <p className="qdm-internal-notes">{quote.rejection_reason}</p>
              </div>
            )}
          ...
          <div className="qdm-footer">
            <button className="qdm-delete-btn" onClick={handleDelete} disabled={actionLoading || loading}>
              <Trash3 size={15} />Eliminar
            </button>
