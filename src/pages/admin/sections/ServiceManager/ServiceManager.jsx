...
const ServiceManager = () => {
  ...
  const handleSubmit = async (customEvent) => {
    ...
    if (customEvent.icon) {
      formData.append("icon", customEvent.icon);
    }

    if (customEvent.remove_icon) {
      formData.append("remove_icon", "true");
    }

    if (customEvent.features) {
      formData.append("features", JSON.stringify(customEvent.features));
    }

    if (customEvent.form_schema) {
      formData.append("form_schema", JSON.stringify(customEvent.form_schema));
    }
    ...
    if (customEvent.remove_images && customEvent.remove_images.length > 0) {
      formData.append(
        "remove_images",
        JSON.stringify(customEvent.remove_images),
      );
    }

    try {
      if (editId) {
        console.log("form_schema que se envía:", customEvent.form_schema);
        console.log(
          "form_schema stringificado:",
          JSON.stringify(customEvent.form_schema),
        );
        formData.forEach((val, key) => console.log("FormData →", key, val));
  ...
  if (loading && services.length === 0) {
    return (
      <div className="service-manager-loading">
