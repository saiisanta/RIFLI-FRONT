...
const BtnSpinner = () => <span className="qw-spinner qw-spinner--sm" />;
...
const DynamicField = ({ field, value, error, onChange }) => {
  ...
  return (
    <div className={`qw-field-group ${error ? 'has-error' : ''}`}>
      ...
      {field.comment && <small className="qw-field-comment">{field.comment}</small>}

      {field.type === 'text' && (
        <input type="text" className="qw-input" value={value ?? ''} onChange={(e) => set(e.target.value)} placeholder={field.placeholder || ''} />
      )}
      {field.type === 'number' && (
        <input type="number" className="qw-input" value={value ?? ''} onChange={(e) => set(e.target.value)} placeholder={field.placeholder || ''} min={field.min} max={field.max} />
      )}
      {field.type === 'textarea' && (
        <textarea className="qw-textarea" value={value ?? ''} onChange={(e) => set(e.target.value)} placeholder={field.placeholder || ''} maxLength={field.maxLength} rows={4} />
      )}
      {field.type === 'select' && (
        <select className="qw-select" value={value ?? ''} onChange={(e) => set(e.target.value)}>
        ...
        <div className="qw-radio-group">
          {(field.options || []).map((opt) => (
            <label key={opt} className={`qw-radio-option ${value === opt ? 'selected' : ''}`}>
              <input type="radio" name={field.id} value={opt} checked={value === opt} onChange={() => set(opt)} />
      ...
      {error && (
        <span className="qw-field-error"><FiAlertCircle size={13} />{error}</span>
      )}
    </div>
  );
};

const QuoteWizard = ({ onSuccess, onApiError }) => {
  ...
  if (submitSuccess) {
    return (
      <div className="qw-success">
        <div className="qw-success-icon"><FiCheckCircle size={48} /></div>
        <h3>¡Solicitud enviada correctamente!</h3>
        <p>Tu solicitud fue recibida. Un técnico la revisará y te enviaremos el presupuesto a la brevedad.</p>
      ...
      <div className="quote-wizard">
        ...
        {step === 1 && (
          <div className="qw-panel">
            ...
            {servicesLoading ? (
              <div className="qw-inner-loading"><div className="qw-spinner" /><span>Cargando servicios…</span></div>
            ) : services.length === 0 ? (
              <div className="qw-empty"><p>No hay servicios disponibles en este momento.</p></div>
            ) : (
                ...
                {services.map((service) => (
                  ...
                  >
                    <div className="qw-service-icon-wrapper">
                      {service.icon ? (
                        <img src={`${API_URL}${service.icon}`} alt={service.type} className="qw-service-icon" />
                      ) : (
                    ...
                    <div className="qw-service-info">
                      ...
                      {(service.form_schema?.fields?.length ?? 0) > 0 && (
                        <span className="qw-service-fields-badge">{service.form_schema.fields.length} preguntas</span>
                      )}
                    </div>
                    <div className="qw-service-check"><FiCheck size={16} /></div>
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="qw-actions">
              <button type="button" className="qw-btn-primary" onClick={() => setStep(2)} disabled={!selectedService}>
                Continuar <FiChevronRight size={18} />
              </button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="qw-panel">
            ...
            {addressLoading ? (
              <div className="qw-inner-loading"><div className="qw-spinner" /><span>Cargando direcciones…</span></div>
            ) : addresses.length === 0 ? (
              <div className="qw-empty">
                ...
                <a href="/perfil" className="qw-btn-secondary">Ir al perfil</a>
              </div>
            ) : (
                  ...
                  >
                    <div className="qw-address-main">
                      <div className="qw-address-alias-row">
                        ...
                        {addr.is_default && <span className="qw-address-default-badge">Principal</span>}
                      </div>
                      ...
                      <p className="qw-address-location">{addr.city}, {addr.province} — CP {addr.postal_code}</p>
                      {addr.additional_info && <p className="qw-address-extra">{addr.additional_info}</p>}
                    </div>
                    <div className="qw-service-check"><FiCheck size={16} /></div>
                  </button>
                ))}
            ...
            <div className="qw-actions">
              ...
              <button type="button" className="qw-btn-primary" onClick={() => setStep(3)}
          ...
          <div className="qw-panel">
            ...
            {schemaFields.length > 0 && (
              <div className="qw-form-block">
                <div className="qw-form-block-title"><span>Información del proyecto</span></div>
            ...
            <div className="qw-form-block">
              ...
              <div className="qw-field-group">
                ...
                <small className="qw-field-comment">Urgencia, preferencias de horario, detalles especiales…</small>
                <textarea
            ...
            {submitError && (
              <div className="qw-error-banner">
                ...
                <span>
                  {typeof submitError === 'string' ? submitError : submitError?.message || 'Error al enviar la solicitud'}
                </span>
              </div>
            )}
            ...
            <div className="qw-actions">
              ...
              <button type="button" className="qw-btn-primary" onClick={handleSubmit} disabled={submitLoading}>
                {submitLoading ? <><BtnSpinner />Enviando…</> : <>Enviar solicitud <FiCheck size={18} /></>}
              </button>
            </div>
          </div>
        )}
