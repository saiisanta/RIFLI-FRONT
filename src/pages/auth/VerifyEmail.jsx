  ...
  const VerifyEmail = () => {
    ...
    useEffect(() => {
      const verify = async () => {
        ...
        if (!token) { setStatus('error'); setMessage('Token de verificación inválido'); return; }
        ...
        try {
          ...
          const timer = setInterval(() => {
            setCountdown(prev => {
              if (prev <= 1) { clearInterval(timer); navigate('/login?verified=true'); return 0; }
              return prev - 1;
            });
          }, 1000);
          ...
          handleApiError(err);
          setStatus('error');
          setMessage(err.error || err.message || 'Token de verificación inválido o expirado');
        }
      };

      verify();
    }, [token, verifyEmail, navigate, handleApiError]);
          ...
          {status === 'success' && (
            <div className="verification-success">
              ...
              <div className="success-content">
                ...
                <p className="success-description">Ya podés iniciar sesión con tu cuenta</p>
              </div>
                ...
                <div className="countdown-circle">
                  <svg className="countdown-svg" viewBox="0 0 100 100">
                    ...
                    <circle className="countdown-progress" cx="50" cy="50" r="45"
                      style={{ strokeDashoffset: `${283 - (283 * (5 - countdown)) / 5}` }} />
                  </svg>
                  <div className="countdown-number">{countdown}</div>
                </div>
                <p className="countdown-text">Redirigiendo al login automáticamente</p>
              </div>
              <Link to="/login" className="btn-link-primary">Ir al Login ahora</Link>
            </div>
          )}
            ...
            <div className="verification-error">
              <div className="error-animation">
                <div className="error-icon-wrapper">
                  <div className="error-icon">
                    <span className="error-line error-line-1" />
                    <span className="error-line error-line-2" />
                  </div>
                </div>
              </div>
              ...
              <div className="error-solutions">
                ...
                <ul className="solutions-list">
                  <li><span className="solution-bullet" /><span>El enlace puede haber expirado (válido por 24 horas)</span></li>
                  <li><span className="solution-bullet" /><span>Ya verificaste tu cuenta anteriormente</span></li>
                  <li><span className="solution-bullet" /><span>El enlace fue copiado incorrectamente</span></li>
                </ul>
              </div>
              <div className="error-actions">
                <Link to="/register" className="btn-outline">Solicitar nuevo enlace</Link>
