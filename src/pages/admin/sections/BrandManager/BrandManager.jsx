...
const BrandManager = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'logo_url' && files && files[0]) {
      const file = files[0];
      setForm((prev) => ({
        ...prev,
        logo_url: file,
  ...
  const handleEdit = (b) => {
    ...
    if (b.logo_url) {
      setImagePreview(`http://localhost:4001${b.logo_url}`);
    } else {
      setImagePreview(null);
    }
  ...
  const currentItems = marcasFiltradas.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  if (loading && brands.length === 0) {
    return (
      <div className="brand-manager-loading">
  ...
  return (
    <div className="brand-manager">
      <BrandHeader
        totalBrands={marcasFiltradas.length}
